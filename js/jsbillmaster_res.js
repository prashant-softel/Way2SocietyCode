	...
	if(sResponse[0].success == 0)
	{
		if(sResponse.length > 1)
		{
			for(var iUnitCount = 1; iUnitCount < sResponse.length; iUnitCount++)
			{
				...
				aryUnitNo.push(sResponse[iUnitCount].unit_no);
			}
...
function displayUnits(aryData)
{
	...
	for(var iUnits = 0; iUnits < aryUnit.length; iUnits++)
	{
		...
		{
			//console.log(aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no));
			iColumnCount = iColumnCount + 1;
			if(res_flag == 1 || rental_flag ==1)
			{
				if((aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no) == 0 ) || (!aryUnitNo.some(item => aryAccHead[iHeader].head.includes(item))))
				{
		...
		if(res_flag == 1 || rental_flag ==1)
		{
		iTable += '<td style="border:1px solid black;"><input type="checkbox" id="chk_' + aryUnit[iUnits].TLedgerID + '" onclick="checkBoxClicked(this);"/><br/>' + aryUnit[iUnits].wing + ' ' + aryUnit[iUnits].unit + '<br/><div style="color:#0000FF;"  onclick="window.open(\'unit_tariff_details.php?&uid=' + aryUnit[iUnits].TLedgerID + '&bill=' + bSupplementaryBill + '\');"><u>Details</u></div></td>';
		}
		else
		{
		...
		for(var iHeader = 0; iHeader < aryAccHead.length; iHeader++)
		{
			...
			if(res_flag == 1 || rental_flag ==1)
			{
				if((aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no) == 0) || (!aryUnitNo.some(item => aryAccHead[iHeader].head.includes(item))))
				{
				iTable += '<td style="border:1px solid black; vertical-align:center;" id="'+ aryUnit[iUnits].TLedgerID + '_' + aryAccHead[iHeader].id + '"><input type="text" style="width:50px;background:#C0C0C0;" value="' + sValue + '" placeholder="Amount" id="amt_' + aryUnit[iUnits].TLedgerID + '_' + aryAccHead[iHeader].id + '" pattern="'+aryAccHead[iHeader].id+'" disabled /><br/><br/><div style="color:#0000FF;" id="detail_' + aryUnit[iUnits].TLedgerID + '_' + aryAccHead[iHeader].id + '" onclick="showDetails(this);"><u>Details</u></div></td>';
				}
			}
			else
			{
		...
		if(res_flag == 1 || rental_flag ==1)
		{
		iTable += '<td style="border:1px solid black; vertical-align:center;"><input type="text" readonly disabled value="' + iBillTotal + '" id="total_' + aryUnit[iUnits].TLedgerID + '" style="background-color:#CCC;width:80px;"/></td>';
		}
		else
		{
		...
		if(freezyear == 0)
		{
			if(res_flag == 1 || rental_flag ==1)
			{
				iTable += '<td style="border:1px solid black; vertical-align:center;"><input type="button" value="Update" id="btn_' + aryUnit[iUnits].TLedgerID + '" onclick="UpdateData(' + aryUnit[iUnits].TLedgerID + ', false);" disabled /></td>';
			}
			else
			{
...
function UpdateAllData()
{
	...
	if(checkIsRangeValid() == true)
	{
		...
		for(var iUnits = 0; iUnits < aryUnit.length; iUnits++)
		{
			if(res_flag == 1 || rental_flag ==1)
			{
				if(document.getElementById('chk_' + aryUnit[iUnits].TLedgerID).checked)
				{
					document.getElementById('update_status').innerHTML = 'Updating Data. Please Wait...';

					bUnitsFound = true;
					UpdateData(aryUnit[iUnits].TLedgerID, true);
				}
			}
			else
			{
		...
		for(var iHeader = 0; iHeader < aryAccHead.length; iHeader++)
		{
			...
			if(res_flag == 1 || rental_flag ==1)
			{
				for(var iUnits = 0; iUnits < aryUnit.length; iUnits++)
				{
					...
					if((aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no) == 0) || ((!aryUnitNo.some(item => aryAccHead[iHeader].head.includes(item)))))
					{
						if(aryUnit[iUnits].TLedgerID == unitID)
						{
							sKey = unitID + '_' + aryAccHead[iHeader].id;
...
function SelectAllUnit(objUnit)
{
	...
	for(var iUnits = 0; iUnits < aryUnit.length; iUnits++)
	{
		if(res_flag == 1 || rental_flag ==1)
		{
			document.getElementById('chk_' + aryUnit[iUnits].TLedgerID).checked = objUnit.checked;
		}
		else
		{
		...
		for(var iHeader = 0; iHeader < aryAccHead.length; iHeader++)
		{
			...
			if(res_flag == 1 || rental_flag ==1)
			{
				if((aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no) == 0) || ((!aryUnitNo.some(item => aryAccHead[iHeader].head.includes(item)))))
				{
					sKeyText = aryUnit[iUnits].TLedgerID + '_' + aryAccHead[iHeader].id;
					sKeyText = 'amt_' + sKeyText;
					sBtn = 'btn_' + aryUnit[iUnits].TLedgerID;
			...
			{
				sKeyText = aryUnit[iUnits].id + '_' + aryAccHead[iHeader].id;
				sKeyText = 'amt_' + sKeyText;
			...
			for(var iUnits = 0; iUnits < aryUnit.length; iUnits++)
			{
				...
				if((aryAccHead[iHeader].head.search(aryUnit[iUnits].unit_no) == 0) || ((!aryUnitNo.some(item => aryAccHead[iHeader].head.includes(item)))))
				{
					if(aryUnit[iUnits].TLedgerID == unitID)
					{
						...
						sKey = 'amt_' + sKey;
						sBtn = 'btn_' + unitID;
						if(chkBox.checked == true)// && fielId == aryAccHead[iHeader].id)
						{
