...
class billdetails_tenant_import 
{
	...
	public function ImportData1($SocietyID)
	{ 
	...
	public function UploadData($fileName,$errorfile)
	{
		...
		if (file_exists($fileName))
		{
			...
			$totalCount = 0;
			$successcount = 0;
            echo "id".$_POST['Period'];
			$prevPeriod = $this->getPrevPeriod($_POST['Period']);
            echo "periodid".$prevPeriod;
		...
		{	
			...
			if($row[0] <> '')
			{
				...
				{  
                    ...
                    $BalanceInvPrincipal_PrevYear = array_search(BalanceInvPrincipal_PrevYear,$row,true);
					$tenant_name = array_search(Tenant_Name,$row,true);
				...
				else
				{
					...
					if(strcmp($QtrMonthNewValue,$periodname)==0 && !$UpdateFlag)
					{
						...
						$society_id=$result[0]['society_id'];
								// echo "call";
						 $search_unit="select unit_id from `unit` where unit_no='".$UnitID."'";
						$result2=$this->m_dbConn->select($search_unit);
                                                $unit_id=$result2[0]['unit_id']; 

                        $search_ledgerid="select ledger_id from `tenant_module` where unit_id='".$unit_id."'AND `tenant_name`like'%".$tenantName."%'";
						$result3=$this->m_dbConn->select($search_ledgerid);
						$ledger_id=$result3[0]['ledger_id'];
					
						
						$billDetails_openingBalance = $BillSubTotal + $BillInterest;
						$assetQuery = "SELECT * FROM `assetregister` WHERE `LedgerID` = '".$ledger_id."' AND `Is_Opening_Balance` = 1";
					...
					{ 
						...
						$society_id=$result[0]['society_id'];
								// echo "call";
						echo $search_unit="select unit_id from `unit` where unit_no='".$UnitID."'";
						$result2=$this->m_dbConn->select($search_unit);
                       echo  $unit_id=$result2[0]['unit_id']; 
						...
						if($UnitID<>'')
						{
							...
							echo $sql = "Select ID,Type from period  where YearID = '" . ($currentYear - 1) . "' and IsYearEnd = 1 ORDER BY ID ASC";
							...
							if($openingBalance)
							{
								if(((float)$CurrentBillAmount) < 0)
								{
									...
									$sqlUpdate = "Update ledger SET opening_balance = '" . abs($CurrentBillAmount) .  "', `opening_type` = '1'  Where id = '" .$ledger_id . "'" ;
									$result = $this->m_dbConn->update($sqlUpdate);
						
									$sqlUpdate = "Update assetregister SET Credit = '" . abs($CurrentBillAmount) . "', Debit = 0 where LedgerID = '" .$ledger_id . "' and Is_Opening_Balance = 1";
									$result = $this->m_dbConn->update($sqlUpdate);
								...
								else
								{
									...
									echo $sqlUpdate = "Update ledger SET opening_balance = '" . abs($CurrentBillAmount) .  "', `opening_type` = '2'  Where id = '" .$ledger_id . "'" ;
									$result = $this->m_dbConn->update($sqlUpdate);
						            
									$sqlUpdate = "Update assetregister SET Debit = '" . abs($CurrentBillAmount) . "', Credit = 0 where LedgerID = '" . $ledger_id . "' and Is_Opening_Balance = 1";
									$result = $this->m_dbConn->update($sqlUpdate);
							...
							else
							{
								if(((float)$CurrentBillAmount) < 0)
								{
									...
									$insertAsset = $obj_register->SetAssetRegister(getDBFormatDate($Date), $ledger_id, 0, 0, TRANSACTION_CREDIT, abs($CurrentBillAmount), 1);
								}
								else
								{
									$sqlUpdate = "Update ledger SET opening_balance = '" . abs($CurrentBillAmount) .  "', `opening_type` = '2'  Where id = '" .$ledger_id . "'" ;
									$result = $this->m_dbConn->update($sqlUpdate);
						
											
									//echo getDBFormatDate($Date);
									$insertAsset = $obj_register->SetAssetRegister(getDBFormatDate($Date), $ledger_id, 0, 0, TRANSACTION_DEBIT, abs($CurrentBillAmount), 1);										
								}
							}
							//$currentYear = $_SESSION['society_creation_yearid'];
							//echo "society_creation_yearid: ".$currentYear."<br>";
							//Your code
						      echo "periodid: ".$MaintainceAmount;
                              
                            if($BillSubTotal==0 && $BillInterest==0)
                            {
                                $MaintainceResult = $this->insertInvoiceDetaildAndRegister($ledger_id,$PeriodID,$BillNumber,$Invamount,$TotalBillPayable,$DueDate2,$society_id,2);	
                            ...
                            else
                            {
                                if($MaintainceAmount <> 0)
                                {
                                    ...
                                    $MaintainceResult = $this->insertBillDetaildAndRegister($ledger_id,$PeriodID,$BillNumber,$BillSubTotal,$BillInterest,$MaintainceAmount,$PrincipalArrears2,$InterestArrears2,$BillDate2,$MaintainceAmount,$DueDate2,$society_id,0);	
                                    if($MaintainceResult == true)
                                    {
                                        $isImportSuccess = true;
                                    }
                                ...
                                {
                                    $Supplementryresult = $this->insertBillDetaildAndRegister($ledger_id,$PeriodID,$BillNumber,$SupplementryAmount,$PrincipalArrears2,$InterestArrears2,$BillDate2,$SupplementryAmount,$DueDate2,$society_id,1);									
							...
							$successcount++;
							$errormsg="Bill for unit: &lt;".$UnitID." &gt;:: period: &lt;".$prevPeriod." &gt;:: period: &lt;".$ledger_id." &gt;imported successfully.";
							$this->obj_utility->logGenerator($errorfile,$rowCount,$errormsg,"I");  	
						}
						else
						{
    ...
    public function insertInvoiceDetaildAndRegister($unit_id,$PeriodID,$BillNumber,$Invamount,$TotalBillPayable,$DueDate2,$society_id,$BillType)
	{
        echo "inside invoice";
		$datam09 = '';
        // echo "duedate".$DueDate2;
        // exit;
       
		$sqlbillregister = "Select  * from `billregister`  where SocietyID = '" . $society_id . "' and PeriodID = '" . $PeriodID . "' and BillType = '".$BillType."' ";
		$resultbillregister = $this->m_dbConn->select($sqlbillregister);
		
		$sqlinvoicedetails = "Select  * from `sale_invoice`  where UnitID = '" . $unit_id . "'  and BillType = '".$BillType."'";
		$resultinvoicedetails = $this->m_dbConn->select($sqlinvoicedetails);
		...
		if(sizeof($resultinvoicedetails) == 0)
		{
            

			$insert_into_invoicedetails="insert into `sale_invoice`(UnitID,Inv_Date,Inv_Number,InvSubTotal,TotalPayable,BillType) 
			values('".$unit_id."','".$DueDate2."','".$BillNumber."','".$Invamount."','".$TotalBillPayable."','".$BillType."') ";
			
			//echo "BillDetail <".$insert_into_billdetails.">";
			
			$data=$this->m_dbConn->insert($insert_into_invoicedetails);
			$isImportSuccess = true;
			//print_r ($data);
		}
		...
		{ 
            echo "calling";
			$invoice_Detail= implode(' | ',$resultinvoicedetails[0]);
			$changeLog="insert into `change_log` (ChangedLogDec,ChangedBy,ChangedTable)  values('<br />Original Record :  ".$invoice_Detail."','".$_SESSION['login_id']."','billdetails')";
			$changeLogDetail=$this->m_dbConn->insert($changeLog);
		
			$updateinvoicedetails = "Update `sale_invoice` set ";
            $updateinvoicedetails .= "Inv_Date ='".$DueDate2."', ";
			$updateinvoicedetails .= "InvSubTotal = '".$Invamount."', ";
			$updateinvoicedetails .= "TotalPayable = '".$TotalBillPayable."' ";
			$updateinvoicedetails .= "where  `UnitID` = '".$unit_id."' and BillType = '".$BillType."'";

            echo $updateinvoicedetails;
			$this->m_dbConn->update($updateinvoicedetails);	
