...
class legalcase
{
	...
	if($_POST['insert']=='Submit' || $_POST['insert'] == "Next" && $errorExists==0)
	{
       ...
       $newImage = $manipulator->resample(1024, 683);
		...
		$array_tenant = explode('_', $_POST['tenant_id']);
		//print_r($array_tenant);	
		//die();
		...
		if($this->isLandLordDB)
		{
			$sql1 = "INSERT INTO `legal_case_history` (`sr_request_id`, `updated_by`, `comment`, `expense_amt`, `status`, `up_hearing_date`, `case_no`,`img`) VALUES ('".$result."', '".$_POST['reported_by']."', '".$_POST['summery']."',0,'Created','".getDBFormatDate($_POST['open_date'])."', '".$_POST['case_no']."','".$image_collection."')";					
		//echo "query:".$sql;  	
		...
		else if($_REQUEST['insert']=='Update' && $errorExists==0)
		{
			...
			//echo $id;
			$image_list=array(); 
			$select = "select `img` FROM `legal_case` WHERE request_id='".$id."'";
       ...
       $newImage = $manipulator->resample(1024, 683);
		...
		//print_r($_REQUEST);
		 ...
		 $up_query="update `legal_case` set `email`='".$_POST['email']."',`phone`='".$_POST['phone']."',`priority`='".$_POST['priority']."',`category`='".$_POST['category']."',`summery`='".$_POST['summery']."', `details`='".$_POST['details']."' ,`img`='$image_collection' where  `request_id`='".$id."' and `society_id`=".$_SESSION['society_id']." ";
			//die();
	...
	{
		////echo "Society_id".$_POST['society_id'];
		$DBName = $this->m_objUtility->getDBName($_POST['society_id']);
		$image_list=array(); 
		for($i=0; $i<count($_FILES['img']['name']); $i++)
			{
				//print_r($_FILES);
		...
		$sql1 = "INSERT INTO `legal_case_history` (`sr_request_id`, `updated_by`, `comment`, `expense_amt`, `status`, `up_hearing_date`, `case_no`,`img`) VALUES ('".$request_id."', '".$_POST['changedby']."', '".$_POST['comments']."','".$_POST['expense_amt']."','".$_POST['status']."','".getDBFormatDate($_POST['upcom_date'])."', '".$_POST['case_no']."', '".$image_collection."')";					
		//echo "query:".$sql;  	
		
		if($_POST['society_id'] != $_SESSION['society_id'])
		{
			$mysqlicon = mysqli_connect(DB_HOST_SER_REQ, DB_USER_SER_REQ, DB_PASSWORD_SER_REQ, $DBName);
			$allresobj = mysqli_query($mysqlicon, $sql1);
			//$result1 = mysqli_fetch_all($allresobj, MYSQLI_ASSOC);
	...
	{
		$sqlSRQuery = "SELECT unit_id FROM `legal_case` WHERE legal_case.`society_id` = '".$_SESSION['society_id']."' AND `request_no` = '".$request_no."'  and  `visibility`='1'";	
		return $this->m_dbConn->select($sqlSRQuery);
	}
	...
	public function GetUnitNoIfNZero($request_no)
	{
		...
		//SELECT s.unit_id, u.unit_no FROM `service_request` s, `unit` u WHERE s.unit_id=u.unit_id AND s.`society_id` = '59' AND s.`request_no` = '53' and s.`visibility`='1'
		 $sqlSRQuery = "SELECT s.`unit_id`, u.`unit_no` FROM `legal_case` s, `unit` u WHERE s.`unit_id` = u.`unit_id` AND s.`society_id` = '".$_SESSION['society_id']."' AND s.`request_id` = '".$request_no."'  and  s.`visibility`='1' and u.unit_id = s.unit_id";	
	...
	public function getRecords($id, $type="")
	{
		...
		if($MemberID <> '')
		{
			...
			
			{
				if( $type == "assign")
				{
					  $sql = "SELECT m1.* FROM legal_case m1 LEFT JOIN legal_case m2 ON (m1.request_no = m2.request_no AND m1.request_id <= m2.request_id) WHERE m2.request_id AND m1.category = '".$catID."' and m1.visibility='1' ";
				}
				else
				{
					 $sql = "SELECT m1.* FROM legal_case m1 LEFT JOIN legal_case m2 ON (m1.request_no = m2.request_no AND m1.request_id < m2.request_id) WHERE  m2.request_id IS NULL  and m1.`visibility`='1' ";
				}
			...
			else
			{
				...
				{
					$sql = "SELECT m1.* FROM legal_case m1 LEFT JOIN legal_case m2 ON (m1.request_no = m2.request_no AND m1.request_id < m2.request_id) WHERE m2.request_id IS NULL AND m1.unit_id = '".$_SESSION['unit_id']."' and m1.visibility='1' ";
				}
		...
		{
			$sql="select * from legal_case where request_no='".$result[$i]['request_no']."' order by timestamp DESC";
	...
	public function getRecords1($id)
	{
		...
		//var_dump($dblist);
		 $sql="select * from legal_case order by timestamp DESC";
...
*/	
	...
	if($_SESSION['role'] && ($_SESSION['role']==ROLE_ADMIN || $_SESSION['role']==ROLE_SUPER_ADMIN || $_SESSION['role']==ROLE_ADMIN_MEMBER))
		{
			$sql="select * from `legal_case` inner join (select request_no, min(timestamp) as ts from `legal_case` group by request_no) maxt on (`legal_case`.request_no = maxt.request_no and `legal_case`.timestamp = maxt.ts)   WHERE legal_case.`society_id` = ".$_SESSION['society_id']." and legal_case.`visibility`='1'  ORDER BY legal_case.request_no  DESC  LIMIT 5 ";
			
		}
		else
		{
			$sql="select * from `legal_case` inner join (select request_no, min(timestamp) as ts from `legal_case` group by request_no) maxt on (`legal_case`.request_no = maxt.request_no and `legal_case`.timestamp = maxt.ts)   WHERE legal_case.`unit_id` = ".$_SESSION['unit_id']." and legal_case.`visibility`='1'  ORDER BY legal_case.request_no  DESC  LIMIT 5 ";
		...
		for($i=0;$i<count($result);$i++)
		{
			$sql="select status from legal_case where request_no='".$result[$i]['request_no']."' order by timestamp DESC";
			$res1 = $this->m_dbConn->select($sql);
			$result[$i]['status']=$res1[0]['status'];
		}
		return $result;
	...
	{
		$sql="select * from legal_case where request_no=".$Id."";
	...
	{ 
		...
		if($_SESSION['role'] && ($_SESSION['role']==ROLE_ADMIN || $_SESSION['role']==ROLE_SUPER_ADMIN || $_SESSION['role']==ROLE_ADMIN_MEMBER || $_SESSION['unit_id']==0))
		{
			...
			$sql = "SELECT legal_case.* FROM `legal_case` WHERE legal_case.`society_id` = '".$soc_id."' AND `".$fieldname."` = '".$request_no."'  and  `visibility`='1'";	
		}
		else
		{
			$sql = "SELECT legal_case.* FROM `legal_case` WHERE legal_case.`society_id` = '".$soc_id."' AND `".$fieldname."` = '".$request_no."'  and  `visibility`='1'";	
	...
	public function getViewDetails1($request_id,$isview=false,$soc_id)
	{
		$DBName = $this->m_objUtility->getDBName($soc_id); 
	...
	public function getLatestStatus($request_id,$societyid)
	{
		
		$DBName = $this->m_objUtility->getDBName($societyid);
	...
	public function getTotalExpense($request_id,$societyid)
	{
		$DBName = $this->m_objUtility->getDBName($societyid);
...
public function deleting($id)
	...
	public function LoadTenantData($tenantId)
	{
		//echo "Landlord".$landLordDB;
		 $sql = "SELECT tenant_id,mobile_no,email FROM `tenant_module` where tenant_id='".$tenantId."'";
		if($this->isLandLordDB){
				$res =$this->m_landLordDB->select($sql);
			}
			//var_dump($this->m_landLordDB);
		//print_r($res);
		return $res;
	}
}
