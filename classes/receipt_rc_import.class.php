...
class receiptImport_rc
{
	...
	public $obj_fetch;
	public $actionPage = "../import_rc_payments_receipts.php";
			 ...
			 for($m=0;$m<$limit;$m++)
			 {
				...
				for($i=0;$i<sizeof($valid_files);$i++)
				{
					...
					else*/
					{
				 	...
				 	if($result <> '')
				 	{
						$this->actionPage="../import_rc_payments_receipts.php";
					 	return $result;
					}
		...
		while (($row = fgetcsv($file)) !== FALSE)
		{
			...
			if($row <> '')
			{
					...
					if($rowCount == 1 || $rowCount == 2)
					{
						...
						$To=array_search(LandlordBankName,$row,true); //Landlord BankName
						$BankName=array_search(TenantBankName,$row,true);
						$BranchName=array_search(TenantBranchName,$row,true);	
						$UnitNo=array_search(UnitNo,$row,true);	
						$TenantName=array_search(TenantName,$row,true);
						...
						$PaymentType=array_search(PaymentType,$row,true);				
						$BillTypeIndex=array_search(BillType,$row,true);			
						$Remark=array_search(Remark,$row,true);	
						
						if(!isset($VDateIndex) || !isset($To)|| !isset($ChequeNo) || !isset($ChequeDateIndex) || !isset($Remark) || !isset($Amount)|| !isset($VNo) || !isset($BillTypeIndex) || !isset($UnitNo) || !isset($PaymentType))
						{
				   ...
				   {	
				   		try 
						{
							...
							{
								$errormsg.="<br><b>Payment Type not accepted &lt;" . $paymentType . "&gt;. please mention correct Payment type.</b>";
							}
						  ...
						  /*	else
							...
							else
							{
								...
								// 	$ledgerName = $unitNo."-".$tenantName."-".$paymentType;
								// }
								// echo $paymentType;
								//echo $ledgerName;
								$ledgerName = $unitNo."-".$tenantName;
								$PaidBy=$this->getLedgerID($ledgerName);
								//echo "Name: " .$PaidBy;
								//exit;
								if($PaidBy=='')
								{
							...
							if($unitNo!='' && ($vNo!='' && is_numeric($vNo)) && $voucherDate!='' && $chequeNo!='' && $chequeDate!='' && $BankID!='' && ($amount!='' &&  is_numeric($amount)) && $PaidBy!='' && (($BillType == 0 ||  $BillType == 1 || $BillType == 2 )))
							{
							 ...
							 echo $BillType;
							  ...
							  $this->obj_ChequeDetails->AddNewValues3($voucherDate,$chequeDate,$chequeNo,$vNo,$systemVoucherNo,1,$amount, $PaidBy, $BankID, $PayerBank, $PayerChequeBranch, $DepositeID, $comments,$BillType,0,0,0,0,0,true,'',0,0,$BatchID,$paymentType); 
