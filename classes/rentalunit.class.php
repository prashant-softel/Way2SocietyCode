...
class rentalunit
{
	...
	public $actionPage= "../rentalunit.php";//"../unit_search.php";
	public $m_dbConn;
	public $m_dbConnRoot;
	public $landLordDB;
	public $isLandLordDB;
		...
		if($_REQUEST['mode']=='Insert' && $errorExists==0)
		{
			if($this->isLandLordDB)
			{
					if($_POST['society_id']<>"" && $_POST['wing_id']<>"" && $_POST['unit_no']<>"" && $_POST['owner_name']<>""  && $_POST['Period']<>"" && $_POST['Period'] > 0 && $_POST['unit_presentation']<>"" && $_POST['unit_presentation'] > 0  && $_POST['ownership_date'] <> "")
					{
						...
						$sql = "select count(*)as cnt from unit where society_id='".$_SESSION['landLordSocID']."' and wing_id='".$_POST['wing_id']."' and unit_no='".$unitLedgerName."' and status='Y'";
						$res = $this->landLordDB->select($sql);
						$mownership = "SELECT * FROM `year`  where `YearID` = '".$_SESSION['society_creation_yearid']."' ";
						
						$mres2 = $this->landLordDB->select($mownership);
						...
						if($res[0]['cnt']==0)
						{
							foreach(explode(',',$_POST['unit_no']) as $k)
							{
								...
								if($k<>"")
								{
									$sql0 = "select count(*)as cnt from unit where society_id='".$_SESSION['landLordSocID']."' and wing_id='".$_POST['wing_id']."' and unit_no='".$unitLedgerName."'";
									
									$res0 = $this->landLordDB->select($sql0);
									...
									if($res0[0]['cnt']==0)
									{
										...
										{
											$sqlInsert = "INSERT INTO `ledger`(`society_id`, `categoryid`, `ledger_name`, `sale`, `receipt`, `opening_balance`,`opening_type`) VALUES ('".$_SESSION['landLordSocID']."', '" . DUE_FROM_MEMBERS . "', '" .$this->m_dbConn->escapeString($unitLedgerName). "', 1, 1, '" . abs($total_opening_balance) . "','1')";												
											$sqlLegerID = $this->landLordDB->insert($sqlInsert);
											$insertAsset = $obj_register->SetAssetRegister_pdc(getDBFormatDate($Date), $sqlLegerID, 0, 0, TRANSACTION_CREDIT, abs($total_opening_balance), 1);
											
										}
										else
										{
											$sqlInsert = "INSERT INTO `ledger`(`society_id`, `categoryid`, `ledger_name`, `sale`, `receipt`, `opening_balance`,`opening_type`) VALUES ('".$_SESSION['landLordSocID']."', '" . DUE_FROM_MEMBERS . "', '" .$this->m_dbConn->escapeString($unitLedgerName). "', 1, 1, '" . abs($total_opening_balance) . "','2')";												
											$sqlLegerID = $this->landLordDB->insert($sqlInsert);
											$insertAsset = $obj_register->SetAssetRegister_pdc(getDBFormatDate($Date), $sqlLegerID, 0, 0, TRANSACTION_CREDIT, abs($total_opening_balance), 1);
										...
										$sql1 = "insert into unit(unit_id,society_id,wing_id,unit_no,property_type,area, flat_configuration,rand_no, sort_order, Location,Plot_No,Makani_No,Premises_No,property_no) values('" . $sqlLegerID . "', '".$_SESSION['landLordSocID']."','".$_POST['wing_id']."','".$this->m_dbConn->escapeString($unitLedgerName)."','".$_POST['property_type']."','".$_POST['area']."','".$_POST['flat_configuration']."','".$rand_no."','".$SortOrderID."','".$_POST['location']."','".$_POST['plot_no']."','".$_POST['makani_no']."','".$_POST['premises_no']."','".$_POST['property_no']."' )";
										
										$res1 = $this->landLordDB->insert($sql1);
										
										
										$sql2="insert into `member_main`(society_id,wing_id,owner_name,unit,ownership_date,alt_address) values('".$_SESSION['landLordSocID']."','".$_POST['wing_id']."','".$this->m_dbConn->escapeString($_POST['owner_name'])."','" . $sqlLegerID . "','".$mres2[0]['BeginingDate']."', '".$this->m_dbConn->escapeString($_POST['permanant_add'])."') ";
										
										$res2 = $this->landLordDB->insert($sql2);
										
										
										$this->UpdateCoOwners($res2, $this->landLordDB->escapeString($_POST['owner_name']),"","");
										...
										$insert_mapping = "INSERT INTO `mapping`(`society_id`, `unit_id`, `desc`, `code`, `role`, `created_by`, `view`) VALUES ('" . $_SESSION['landLordSocID'] . "', '" . $sqlLegerID . "', '" . $unitLedgerName . "', '" . getRandomUniqueCode() . "', '" . ROLE_MEMBER . "', '" . $_SESSION['login_id'] . "', 'MEMBER')";
										$result_mapping = $this->m_dbConnRoot->insert($insert_mapping);
										
										$this->actionPage="../list_member2.php";
									}
								}
							}
					...
					foreach(explode(',',$_POST['unit_no']) as $k)
					{
						...
						if($k<>"")
						{
							...
							if($res0[0]['cnt']==0)
							{
								...
								$sql1 = "insert into unit(unit_id,society_id,wing_id,unit_no,property_type,area, flat_configuration,rand_no, sort_order, Location,Plot_No,Makani_No,Premises_No,property_no) values('" . $sqlLegerID . "', '".$_SESSION['society_id']."','".$_POST['wing_id']."','".$this->m_dbConn->escapeString($unitLedgerName)."','".$_POST['property_type']."','".$_POST['area']."','".$_POST['flat_configuration']."','".$rand_no."','".$SortOrderID."','".$_POST['location']."','".$_POST['plot_no']."','".$_POST['makani_no']."','".$_POST['premises_no']."','".$_POST['property_no']."' )";
	...
	public function getTenantWing($id)
	{
		...
		return $this->combobox07($query, $id,$isLandLordDB);
						
	}

	public function combobox07($query, $id,$dbselected= false)
	{
