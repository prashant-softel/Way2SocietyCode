...
$objFetchData = new FetchData($dbConn,$dbConnRoot,$m_landlord);
$rec=false; 
if($_SESSION['res_flag'] == 1)
{
	$objFetchData->GetSocietyDetails($_SESSION['landLordSocID'], true);
	$rec =true;
}
else
{
...
$obj_memberDuesRegular= new memberDuesRegular($dbConn,$dbConnRoot,$m_landlord);
...
?>
<script>
	window.location.href="common_period.php?tenantdues&temp";
</script>			
<?php }
else
{	
	$show_dues_details=$obj_memberDuesRegular->getTenantDuesRegular($_POST['dues_range'], $_POST['wing'],$_POST['to_date'],$_POST['BillType'],$rec);
	if($show_dues_details == '')
	{
	?>	
	<script>
		window.location.href="common_period.php?tenantdues&fail";
	</script>		
	<?php	
	}
}
...
<div style="border: 1px solid #cccccc;">
 ...
 <input type="hidden" id= 'temp_data' value="<?php echo $templateData[0]['template_data']; ?>">
        ...
        <div id="bill_subheader" style="text-align:center;">
            <div style="font-weight:bold; font-size:16px;">Dues From Tenant-Regular Bill</div>
  ...
  <td colspan="8" id="showbutton1" style="text-align: center;border:0px; display:block;"><input type="button" id="btn_notice" onClick="sendNotice();" value="Send overdue notice to selected units" style="color: #fff;background-color: #337ab77a;border-color: #2e6da4;padding: 6px 12px;font-size: 14px;font-weight: 400;line-height: 1.42857143;cursor: pointer;border: 1px solid transparent;border-radius: 4px;" disabled/></td>
        ...
        <table  style="width:100%;font-size:14px; " class="table table-striped">
        	<tr>
                ...
                <th style="text-align:center;  width:12%;border: 1px solid #cccccc;" >Building Name</th>
                <th style="text-align:center;  width:15%;border: 1px solid #cccccc;">Unit No.</th>
                <th style="text-align:center;  border: 1px solid #cccccc;" colspan="3">Tenant Name</th>
            ...
            <?php				
				//var_dump($show_dues_details);
				if($show_dues_details <>"")
				{
					$count=1;
					$totalAmount = 0;
					for($i= 0; $i< sizeof($show_dues_details);$i++)
					{
						if($show_dues_details[$i]['totaldues'] > "0" && $show_dues_details[$i]['days'] > 0 )
						{
							$totalAmount = $totalAmount+$show_dues_details[$i]['totaldues'];
							?>
						<tr>
                    		<td><?php echo $count ?></td>
                    		<td><?php echo $show_dues_details[$i]['wing'] ?></td>
                    		<td><?php echo $show_dues_details[$i]['unit_no'] ?></td>
                    		<td colspan="3"><?php echo $show_dues_details[$i]['tenant_name'] ?></td>
                    		<td colspan="3" style="text-align:right"><?php echo number_format($show_dues_details[$i]['totaldues'],2) ?></td>
                   			<td colspan="3" style="text-align:right"><?php echo number_format($show_dues_details[$i]['totaldues'],2) ?></td>
                    		<td colspan="3" style="text-align:right"><?php echo $show_dues_details[$i]['days'] ?></td>
                    	</tr>	
					...
					?>
                    <tr>
                    <td colspan="6" style="text-align:center"><b> Total Amount</b></td>
                    <td colspan="3" style="text-align:right"><b><?php echo number_format($totalAmount,2) ?></b></td>
                    <td colspan="3" style="text-align:right"><b> <?php echo number_format($totalAmount,2) ?></b></td>
                    <td colspan="3"> </td>
                    </tr>
				<?php }
