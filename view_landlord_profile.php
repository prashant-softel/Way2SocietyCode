...
include_once "classes/view_landlord_profile.class.php" ;
...
$obj_tenant = new rentaltenant($m_dbConn,$m_dbConnRoot);
...
$obj_view_member_profile = new view_landlord_profile($m_dbConn);
    ...
    <td colspan="6" style="font-weight: bold;text-align: center;">
        ...
        <?php
           if ($_SESSION['res_flag'] == 1) {
                echo '<i class="fa fa-group" style="font-size: 14px;">&nbsp;</i><b><u> Details of Landlords </u></b>&nbsp;<i class="fa fa-group" style="font-size: 14px;"></i>';
            } else {
                echo '<i class="fa fa-group" style="font-size: 14px;">&nbsp;</i><b><u> Details of Flats </u></b>&nbsp;<i class="fa fa-group" style="font-size: 14px;"></i>';
            }
        ?>
...
--><tr>
    ...
    <?php if( $TenantDetails[0]['tenant_id']=='')
     ...
     <tr height="30" bgcolor="#E8E8E8">
      <th width="280">Name on the Lease Document</th>
                      ...
                      <!--  <th width="100">Mobile</th>
                        ...
                        <th width="150">Contract Value</th>
                        <th width="150">Annual Rent</th>
     ...
     <?php }
        ...
        <?php
        $date = date("Y-m-d");
        $tenant_list = $TenantDetails;
        for($i = 0; $i < sizeof($tenant_list);$i++){
            if($tenant_list[$i]['end_date'] > $date){$status = "Active"; }else{$status = "Expired"; } ?>
            <tr height="25" bgcolor="#BDD8F4">
            <td><a onClick="window.location.href='tenant.php?mem_id=<?php echo $_GET['id']; ?>&tik_id=<?php echo time(); ?>&view=<?php echo $tenant_list[$i]['tenant_id']; ?>'"><?php echo $tenant_list[$i]['tenant_name'] ?></a></td>
            <td><?php echo getDisplayFormatDate($tenant_list[$i]['start_date']);?></td>
            <td><?php echo getDisplayFormatDate($tenant_list[$i]['end_date']);?></td>
            <!--<td><?php// echo $TenantDetails[0]['mobile_no']?></td>
            <td><?php //echo $TenantDetails[0]['email']?></td>-->
            <td><?php echo $tenant_list[$i]['contract_value']?></td>
            <td><?php echo $tenant_list[$i]['annual_rent']?></td>
