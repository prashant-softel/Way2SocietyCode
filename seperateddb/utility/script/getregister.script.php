		...
		try
			...
			for($iCount = $startNo; $iCount <= $endNo; $iCount++)
			{
				...
				else
				{
					...
					echo  "SELECT COUNT(ID) as NEFT , VoucherDate FROM `chequeentrydetails` where DepositID = -2 and `VoucherDate` between '".$fromdate."' and '".$todate."' GROUP BY MONTH(VoucherDate) order by VoucherDate";
					$finalarray['NEFT'] = getResult($mMysqli, "SELECT COUNT(ID) as NEFT , VoucherDate FROM `chequeentrydetails` where DepositID = -2 and `VoucherDate` between '".$fromdate."' and '".$todate."' GROUP BY MONTH(VoucherDate) order by VoucherDate");
					
					
					$finalarray['CASH'] = getResult($mMysqli, "SELECT COUNT(ID) as CASH , VoucherDate FROM `chequeentrydetails` where DepositID = -3 and `VoucherDate` between '".$fromdate."' and '".$todate."' GROUP BY MONTH(VoucherDate) order by VoucherDate");
					
					$finalarray['PayTM'] = getResult($mMysqli, "SELECT COUNT(ID) as PayTM , VoucherDate FROM `chequeentrydetails` where DepositID = -4 and `VoucherDate` between '".$fromdate."' and '".$todate."' GROUP BY MONTH(VoucherDate) order by VoucherDate");
					
					$finalarray['CHEQUE'] = getResult($mMysqli, "SELECT COUNT(ID) as CHEQUE , VoucherDate FROM `chequeentrydetails` where  DepositID NOT IN (-2,-3,-4) and `VoucherDate` between '".$fromdate."' and '".$todate."' GROUP BY MONTH(VoucherDate) order by VoucherDate");
